				...
				if($stmt = mysqli_prepare($link, $sql)){
					...
					if(mysqli_stmt_execute($stmt)){
						...
						if($stmt = mysqli_prepare($link, $sql)){
							...
							$time = date("h:iA");
							if(mysqli_stmt_execute($stmt)){
								header("Location: equipTech.php?msg=equipadded");
							}
							else{
			...
			mysqli_stmt_bind_param($stmt, "sssss", $_POST['serialno'], $_POST['cbModel'], $_POST['desc'], $_POST['cbStatus'], $edit);
				if(mysqli_stmt_execute($stmt)){
					...
					if($stmt = mysqli_prepare($link, $sql)){
						...
						$time = date("h:iA");
						if(mysqli_stmt_execute($stmt)){
								header("Location: equipTech.php?msg=equipupdated");
							}
							else{
								echo "Error on inserting logs. Please try again later.";
							}
	...
	if($stmt = mysqli_prepare($link, $sql)){
	  ...
	  if(mysqli_stmt_execute($stmt)){
		...
		if($stmt = mysqli_prepare($link, $sql)){
			...
			$time = date("h:iA");
			if(mysqli_stmt_execute($stmt)){
				header("Location: equipTech.php?msg=equipdeleted");
			}
			else{
				echo "Error on inserting logs. Please try again later.";
			}
	  ...
	  if(mysqli_stmt_execute($stmt)){
		...
		if(mysqli_num_rows($result) == 1){
		  ...
		  if($oldpass != $row['password']){
			...
			header('location: equipTech.php#passnotmatch');
		  }
		  else{
			...
			if($stmt = mysqli_prepare($link, $sql)){
				mysqli_stmt_bind_param($stmt, "ss", $newpass, $edit);
					if(mysqli_stmt_execute($stmt)){
					  header('location: equipTech.php#changedpass');
					}
					else{
						echo "Error on update statement";
					}
  ...
  if(isset($_POST['btnPasslogs'])){
	...
	if($stmt = mysqli_prepare($link, $sql)){
	  ...
	  if(mysqli_num_rows($result) == 1){
		...
		$_SESSION['oldPass'] = trim($_POST['oldPass']);
		header('location: logsEquipTech.php#passnotmatch');
		}
		else{
		$sql = "UPDATE tblaccount SET password = ? WHERE username = ?";
		if($stmt = mysqli_prepare($link, $sql)){
		  mysqli_stmt_bind_param($stmt, "ss", $newpass, $edit);
			if(mysqli_stmt_execute($stmt)){
			  header('location: logsEquipTech.php?msg=changedpass');
			}
			else{
			  echo "Error on update statement";
			}
	...
	if($stmt = mysqli_prepare($link, $sql)){
	  mysqli_stmt_bind_param($stmt,"s", $delete );
	  if(mysqli_stmt_execute($stmt)){
		  header('location: logsEquipTech.php?msg=logdeleted');
	  }
	  else {
	  echo "Error on delete statement. Please Try Again.";
	  }		
