...
<html>
  ...
  <body>
    ...
    <div class="d-flex align-items-stretch">
      <div id="sidebar" class="sidebar py-3 bg-dark">
        ...
        <ul class="sidebar-menu list-unstyled">
                <li class="sidebar-list-item"><a href="ticketUser.php" class="sidebar-link text-muted active"><i class="o-paper-stack-1 mr-3 text-gray"></i><span>Ticket</span></a></li>
      ...
      <div class="page-holder w-100 d-flex flex-wrap">
        ...
        <section>
            ...
            <a href="logsTicketUser.php" class="btn btn-info">Logs</a>
            <a href="ticketUser.php" class="btn btn-warning">Refresh</a>
            </div>
            <?php
              if(isset($_GET['msg'])){
                $msg = $_GET['msg'];
                ...
                if($msg == "changedpass"){
                  ...
                  echo "<a href='logsTicketUser.php' class='close'> <span aria-hidden='true'>&times;</span></a></div>";
                }
                elseif($msg == "ticketadded"){
                  echo "<div class='alert alert-success alert-dismissible fade show text-center' role='alert'>";
                  echo "Ticket added successfully!";
                  echo "<a href='ticketUser.php' class='close'> <span aria-hidden='true'>&times;</span></a></div>";
                }
                elseif($msg == "ticketexist"){
                  echo "<div class='alert alert-warning alert-dismissible fade show' role='alert'>";
                  echo "Ticket already exist!";
                  echo "<a href='ticketUser.php' class='close'> <span aria-hidden='true'>&times;</span></a></div>";
                }
                elseif($msg == "ticketupdated"){
                  echo "<div class='alert alert-success alert-dismissible fade show' role='alert'>";
                  echo "Ticket updated!";
                  echo "<a href='ticketUser.php' class='close'> <span aria-hidden='true'>&times;</span></a></div>";
                }
                elseif($msg == "ticketdeleted"){
                  echo "<div class='alert alert-success alert-dismissible fade show' role='alert'>";
                  echo "Ticket deleted!";
                  echo "<a href='ticketUser.php' class='close'> <span aria-hidden='true'>&times;</span></a></div>";
                }
                ...
                <div class="card">
                  <div class="card-header">
                    <div class="form-group">
                      <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#AddTicket">Add</button>
                      <div class="float-right">
                        ...
                        <select id="prob" class="custom-select srchOp prob" onchange="prob(this)" name="cbProb">
                        <option hidden>Problem</option>
                  ...
                  <?php
                    ...
                    $sql = "SELECT * FROM tblticket Where createdBy = '".$_SESSION['account']."' order by date DESC, time DESC ";
                        ...
                        <div class="card-body">   
                          ...
                          <table class="table table-hover card-text">
                            ...
                            ?>
                                <td> 
                                    ...
                                    <?php
                                      if($row['status'] == "Pending"){
                                        ?>
                                        <a href="#edit<?php echo $ticketid;?>" data-toggle="modal" class="btn btn-info">Edit</a>
                              ...
                              <!-- Edit Modal -->
                              <div class="modal fade" id="edit<?php echo $ticketid;?>" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                  <div class="modal-content">
                                    <div class="modal-header">
                                    ...
                                    <div class="modal-body">
                                    <form action="processUserTicket.php" method="post">
                                      <label>Ticket ID:</label>
                                      <input type="text" class="form-control" name="ticketid" id="" value="<?php echo $ticketid; ?>" readonly="readonly">
                                      <label>Problem:</label>
                                        <select id="probedit" class="custom-select" name="cbProb">
                                            <option value="<?php echo $row['problem']?>" hidden><?php echo $row['problem']?></option>
                                            <option value="Hardware">Hardware</option>
                                            <option value="Software">Software</option>
                                            <option value="Connection">Connection</option>
                                          </select>
                                      <label>Category:</label>
                                        <select id="catedit" class="custom-select" name="cbCat">
                                          <option value="<?php echo $row['problem']?>" hidden><?php echo $row['category']?></option>
                                        </select>
        ...
        <!--Add Modal -->
        <div class="modal fade" id="AddTicket" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
          <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
              ...
              <div class="modal-body">
              <form action="processUserTicket.php" method="post" name="add" onsubmit="return IsEmpty()">
                <label>Problem:</label>
                <select id="probadd" class="custom-select" name="cbProb">
                  <option value="none" hidden>Problem</option>
                  <option value="Hardware">Hardware</option>
                  <option value="Software">Software</option>
                  <option value="Connection">Connection</option>
                </select>
                ...
                <label>Category:</label>
                <select id="catadd" class="custom-select" name="cbCat">
                  <option value="none" hidden>Category</option>
                </select>
                <p id="y" class="text-danger"></p>
                <label for="my-textarea">Description:</label>
                <textarea id="my-textarea" class="form-control" name="desc" rows="3"></textarea>
        ...
        <div class="modal fade" id="changePass" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                  <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                      <form action = "processUserTicket.php" method="post" name="pass"onsubmit="return password()">
                          <div class="form-group">
    ...
    location.assign("ticketUser.php");
});
...
if(window.location.href.indexOf('#changedpass') != -1) {
  swal({
  text: "Password changed!",
  icon: "success",
  button: "Close"
}).then(function() {
window.location.href = "ticketUser.php";
});
}

});
$(document).ready(function () {
    $("#probedit").change(function () {
        var val = $(this).val();
        if (val == "Hardware") {
            $("#catedit").html("<option value='Laptop'>Laptop</option><option value='CPU'>CPU</option><option value='Monitor'>Monitor</option><option value='Keyboard'>Keyboard</option><option value='Mouse'>Mouse</option><option value='Speaker'>Speaker</option><option value='Projector'>Projector</option><option value='Printer'>Printer</option>");
        } else if (val == "Software") {
            $("#catedit").html("<option value='Application'>Application</option><option value='OS'>OS</option>");
        } else if (val == "Connection") {
            $("#catedit").html("<option value='Local'>Local</option><option value='Internet'>Internet</option>");
        } 
    });
});
$(document).ready(function () {
...
function password() {
  ...
  var cbProb=document.add.cbProb.value;
  var cbCat=document.add.cbCat.value;
  var desc = document.forms["add"]["desc"].value;
  if (cbProb == "none") {
    document.getElementById("x").innerHTML = "Select a problem"; 
    return false;
  }
  else{
  ...
  if (cbCat == "none") {
    document.getElementById("y").innerHTML = "Select a category"; 
    return false;
  }
  else{
  ...
  if (desc == "") {
    document.getElementById("z").innerHTML = "cannot be blank"; 
    return false;
  }
  else{
